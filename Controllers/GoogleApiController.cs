...
public class GoogleApiController : ControllerBase
{
    ...
    public async Task<IActionResult> Gemini()
    {
        ...
        try
        {
            var url = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={Uri.EscapeDataString(key)}";
    ...
    /// <summary>GET /api/google/maps - Test Maps (Static Map) to verify key works for Maps JavaScript API.</summary>
    [HttpGet("maps")]
    public async Task<IActionResult> Maps()
    {
        ...
        try
        {
            var url = $"https://maps.googleapis.com/maps/api/staticmap?center=0,0&zoom=1&size=1x1&key={Uri.EscapeDataString(key)}";
            using var httpClient = new HttpClient();
            httpClient.Timeout = TimeSpan.FromSeconds(10);
            var response = await httpClient.GetAsync(url);
            if (response.IsSuccessStatusCode)
                return Ok(new { status = "ok", message = "Maps API key is valid (Static Map check). Use it in Maps JavaScript API on the frontend.", service = "Maps" });
            var responseText = await response.Content.ReadAsStringAsync();
